...
namespace OnlineShopMvcWeb.Areas.Administration.Controllers
{
    public class ManageCategoriesController : Controller
    {
        ...
        {
            var categories = db.Categories.Where(x => x.ParentId == 0).OrderBy(x => x.Name).ToList();
            categories.Add(new Category() { Id = 0, Name = "[Select a category]", ParentId = 0, SortOrder = -1, Status = true });

            ViewBag.ParentId = new SelectList(categories.OrderBy(x =>x.SortOrder), "Id", "Name");
            return View();
        }
